...
class DQN(nn.Module):
   ...
   def forward(self, x):
       return self.fc(x)
class DQNAgent:
    def __init__(self, state_dim, action_dim=2, device='cpu', cql_alpha=1e-4, lr=1e-4, gamma=0.99):
        self.device = device
    ...
    def train_offline(self, state_list, action_list, reward_list, next_state_list, done_list,
                      epochs=100, batch_size=32, update_frequency=50):
...
def main():
   ...
   if len(state_list) > 0:
       print("\n=== DQN 학습 시작 ===")
       agent = DQNAgent(state_dim=len(state_list[0]), action_dim=2, device)
       agent.train_offline(state_list, action_list, reward_list, next_state_list, done_list)
